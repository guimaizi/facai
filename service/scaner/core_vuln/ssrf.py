...
class SSRFScanner:
    ...
    def __init__(self, dnslog_url=None, project_name=None, timeout=5):
    ...
    def scan(self, request_data, params_list=None, send_request_func=None):
        ...
        Returns:
            list - 发现的漏洞列表
                [
                    {
                        'vuln_type': 'SSRF疑似漏洞',
                        'ssrf_type': '无回显SSRF（需验证DNSLog）',
                        'param': 'url',
                        'position': 'POST',
                        'payload': 'http://ssrf123.dnslog.com',
                        'dnslog_id': 'ssrf123',
        ...
        for param_info in params_list:
            # 只测试URL类型参数
            if param_info.get('param_type') != 'url':
                continue
            
            try:
                # 生成唯一的DNSLog标识（ssrf + 5位随机字母数字）
                dnslog_id = f"ssrf{self._generate_hash(5)}"
                
                # 使用dnslog_url模板，替换{hash}为标识符
                # dnslog_url格式: "http://www.dnslog.com/{hash}"
                payload = self.dnslog_url.replace('{hash}', dnslog_id)
                
                # 使用 ParamHandler.set_param_value 修改参数（替换模式）
                test_request = self.param_handler.set_param_value(
                    request_data, 
                    param_info['param_name'], 
                    payload, 
                    mode=1  # 替换模式（URL参数需要替换原值）
                )
                ...
                if self.project_name:
                    try:
                        ...
                        print(f"[SSRF Scanner] 已保存盲打日志: {dnslog_id}")
                    except Exception as e:
                        print(f"[SSRF Scanner] 保存盲打日志失败: {e}")
                
                # 不添加到 vulnerabilities，SSRF盲打只记录日志
...
if __name__ == "__main__":
    # 测试代码
    scanner = SSRFScanner(dnslog_url="http://www.dnslog.com/{hash}")
    
    # 测试HTTP请求
    test_request = {
        "url": "http://127.0.0.1/test631?url=http://example.com",
        "method": "POST",
        "headers": {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
        },
        "body": "callback=https://api.test.com"
    }
